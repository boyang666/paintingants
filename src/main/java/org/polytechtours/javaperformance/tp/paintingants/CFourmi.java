...
public class CFourmi {
  ...
  */
  ...
  public synchronized void deplacer() {
	...
	blue = mApplis.mBaseImage.getRGB(i, j) & 0xff;
	green = (mApplis.mBaseImage.getRGB(i, j) & 0xff00) >> 8;
	red = (mApplis.mBaseImage.getRGB(i, j) & 0xff0000) >> 16;
    } else {
      //lCouleur = new Color(mPainting.getCouleur(i, j).getRGB());
	blue = mPainting.getCouleur(i, j).getRGB() & 0xff;
	green = (mPainting.getCouleur(i, j).getRGB() & 0xff00) >> 8;
	red = (mPainting.getCouleur(i, j).getRGB() & 0xff0000) >> 16;
    }
    if (testCouleur(red, green, blue)) {
      dir[0] = 1;
    }
