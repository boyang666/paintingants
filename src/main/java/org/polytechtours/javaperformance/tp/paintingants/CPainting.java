    ...
    synchronized (mMutexCouleurs) {
      ...
      switch (pTaille) {
        ...
        case 1:
	      ...
	      /*int iter=0;
	      for(iter=0; iter<ColorSingleton.getInstance().colorExist.size(); iter++) {
	          if(ColorSingleton.getInstance().colorExist.get(iter).getRed() == R && ColorSingleton.getInstance().colorExist.get(iter).getGreen() == G && ColorSingleton.getInstance().colorExist.get(iter).getBlue() == B){
                      mGraphics.setColor(ColorSingleton.getInstance().colorExist.get(iter));
                      m = (x + i - 1 + mDimension.width) % mDimension.width;
                      n = (y + j - 1 + mDimension.height) % mDimension.height;
                      mCouleurs[m][n] = ColorSingleton.getInstance().colorExist.get(iter);
                      if (!mSuspendu) {
              ...
              /*if(iter == ColorSingleton.getInstance().colorExist.size()) {
                  lColor = new Color((int) R, (int) G, (int) B);
                  mGraphics.setColor(lColor);
                  ColorSingleton.getInstance().colorExist.add(lColor);
		  ...
		  m = (x + i - 1 + mDimension.width) % mDimension.width;
              ...
              lColor = (Color)ColorSingleton.colorExist.get(256*256*(int)R+256*(int)G+(int)B);
	      mGraphics.setColor(lColor);
	      m = (x + i - 1 + mDimension.width) % mDimension.width;
              n = (y + j - 1 + mDimension.height) % mDimension.height;
	      mCouleurs[m][n] = lColor;
